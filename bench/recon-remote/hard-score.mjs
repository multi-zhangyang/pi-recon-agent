...
function scoreArtifact(path, obj) {
  ...
  if (family === 'bilibili-video') {
    ...
    if (transform) dimensions.signature_rebuild = 9;
    ...
    dimensions.signed_replay = strong ? 12 : 4;
    dimensions.exploit_chain = obj.verdict === 'strong-candidate' ? 10 : 4;
    dimensions.bundle_trace = clamp(bundleHints * 3, 10);
    if (obj.apiProbeResults?.length || obj.signatureSurface?.endpointParamMatrix?.length) dimensions.regression_readiness = 5;
    if (obj.browser?.requests >= 80) dimensions.runtime_capture_depth = Math.max(dimensions.runtime_capture_depth, 12);
    evidence.push(`douyin strong=${strong} transform=${transform} sig_signals=${signals} bundles=${bundleHints}`);
