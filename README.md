...
- `gate:cross-session-multi-compact-matrix` 验证 `CrossSessionMultiCompactMatrixGateV1`：五轮 compact/resume 跨 session 保持 exact contextPath/hash 优先级，multi-provider 与 remote provider continuation sample matrix 必须接在 exact resume 后，operator/proof-loop closure 必须进入 done|blocked|exhausted 且 terminal rows 不重开。
