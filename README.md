...
> 当前收口状态：REPI 已能正常作为专业逆向/渗透任务组织 agent 使用；并行计划、agent-dogfood subagent runtime manifest、AgentDogfoodFailureSignatureBindingGateV1、AgentDogfoodStructuredClaimMergeGateV1、AutonomousHardeningGapLedgerV1、AutonomousClosureReadinessGateV1、CapabilityClaimReleaseBundleGateV1、ReleaseCiPipelineGateV1、ReleaseEvidenceIndexGateV1、SwarmProviderManifestParityGateV1（含 ProviderBackedLongWindowSharedMergeLedgerV1 / ProviderWorkerExtendedRetryManifestChainV1）、WorkerProviderRepairRollbackUnificationGateV1（含 RemoteProviderStateChangingRepairMatrixV1 / DeepCompoundProviderRepairCompletionChainV1）、LiveConflictArbitrationMatrixGateV1（含 provider-backed long-window conflict matrix 与 extended synthesizer topic parse matrix）、AutonomousRuntimeBatchV1 strict gate、agent-dogfood / re_swarm / compound-frontier runtime ClaimLedgerEventV1、上下文 pack/resume、supervisor 分工验证、strict claim release gate、strict failure/repair schema fixture、`RepairRollbackPolicyV1` baseline/allowlist/regression/rollback gate、provider/worker repair rollback unification gate、`ToolCallTraceLedgerV1` append-only tool trace、runtime failure/repair ledger hooks、离线控制面 gates 已接入。最终输出不会只靠叙述放行：`re_supervisor`、`re_compiler final`、`re_complete audit` 都会读取 strict claim marker，并在 required claim gap 未闭合时阻断最终发布。
...
- failure/repair runtime ledger：`FailureLedgerEventV1`、`RepairQueueItemV1` strict schema、strict fixture、duplicate signature/attempt 去重检查、hard-eval 离线样例，`re_replayer` / `re_autofix` / `re_operator` / `re_proof_loop` failed|blocked row 到 `~/.repi/agent/recon/evidence/failures/ledger.jsonl`、`~/.repi/agent/recon/evidence/repairs/queue.jsonl` 的 append-only 写入 hooks，以及 compound-frontier、agent-dogfood role retry、plan-only invalid fixture 的 failure/repair 输出；`AgentDogfoodFailureSignatureBindingGateV1` 会把 agent-dogfood 失败 role 的 `failureSignatureBinding`、`failureLedgerEventId`、`repairQueueItemId`、`retryBudget` 和 `dedupeWindow` 写回对应 runtime manifest 与 manifest index。
...
- AutonomousHardeningGapLedgerV1：`npm run gate:autonomous-hardening-gap-ledger` 会运行 `autonomy-control-plane --json` 并验证 remaining top-autonomous gaps 不再只是自然语言 hardening_needed，而是机器可执行 gap ledger：每个 gap 必须有 pillar、closureGate、regressionCommands、nextCommand、artifacts、acceptanceCriteria、missingRuntimeProof 和 promotionPolicy；只要存在未 closed gap，`topAutonomousDefinition` 必须保持 false；所有 gap closed 且 artifact-backed 后才允许为 true。
- AutonomousClosureReadinessGateV1：`npm run gate:autonomous-closure-readiness` 会把 `AutonomousHardeningGapLedgerV1` 中每个 `closureGate` 拉成总控 readiness matrix，逐项验证 package script、gate script、top harness child gate、autonomy control-plane contract、README/docs、以及 `--strict --no-write` 可执行结果，防止 gap 只在文档中 ready、没有进入真实 release harness。
- CapabilityClaimReleaseBundleGateV1：`npm run gate:capability-release-bundle` 会生成 release 能力声明 bundle，把 README/发布口径中的“独立产品边界、专业控制面、closure readiness、top autonomous artifact-backed”四类 claim 绑定到 command evidence、source file sha256、autonomy JSON 和 closure readiness summary；缺命令证据、命令失败、secret 泄漏、narrative-only promotion 或在 ready gaps 下误置 `topAutonomousDefinition=true` 都会失败。
- ReleaseCiPipelineGateV1：`npm run gate:release-ci-pipeline` 会静态审计 `.github/workflows/repi-harness.yml` 与 `docs/reverse-agent/repi-harness.github-actions.yml`，确认 CI 显式按顺序运行 product boundary、profile isolation、product surface、closure readiness、capability release bundle、top harness、repo check 和 no-diff guard，并且不依赖 live provider secret。
- ReleaseEvidenceIndexGateV1：`npm run gate:release-evidence-index` 会把 autonomy control plane、closure readiness、capability release bundle、release CI pipeline、source file sha256 和 command output hash 合并成 `ReleaseEvidenceIndexV1`，并用 hash-chain rows 与 secret-free 检查保证发布证据可追溯。
- runtime ClaimLedgerEventV1：agent-dogfood、re_swarm 与 compound-frontier 统一输出 `artifact_handoff → claim → validation → challenge → resolution` 哈希链；agent-dogfood 每个 role / synthesizer attempt 会输出 `*.runtime-manifest.json`；失败 role 还会把 `AgentDogfoodFailureSignatureBindingV1` 写入 runtime manifest 和 `subagent-runtime-manifests.json`；`AgentDogfoodStructuredClaimMergeGateV1` / `npm run gate:agent-dogfood-structured-claims` 会生成 `structured-claim-merge.json`，把 role/synthesizer summary 降成 structured claim rows，只有带 runtime manifest sha256、JSON query、verifierPass 且无 unresolved challenge 的 claim 才能进入 finalClaims，narrative-only 输出只能作为 observation，re_swarm run 为每个 worker 写 `SubagentRuntimeManifestV1`、stdout/stderr sha256、sessionDir、toolCallDigest 与 `*-subagent-runtime-manifests.json`，compound-frontier 同步写 `claim-ledger.jsonl`；`npm run gate:runtime-claim-ledger` 会把最新 runtime ledger 适配进 `validate-claim-ledger.mjs` 的 strict validator，缺少 source 时会自动跑 bounded agent-dogfood plan-only native ledger、bounded `reSwarmLiveProbe` 与 compound-frontier use-latest native ledger，并在 `runtimeLedgerQuality` 中汇总 artifact sha256、event type count、tip hash、hash-chain 与 strict validator 摘要；`RuntimeLedgerQualityGateV1` / `npm run gate:runtime-ledger-quality` 会把这些字段提升为独立质量门禁，防止 role/worker/compound claim 只停留在叙述层。
- Cross-session resume live：新增 `CrossSessionResumeLiveV1` hard-eval，把 context pack/resume 从单进程 schema gate 推进到跨 session runtime。`npm run gate:cross-session-resume-live` 会用 session A 创建 mission/map/memory 并 `re_context pack`，用 session B 按原始 `contextPath` exact resume，验证 `loadedBy=contextPath`、contextSha256/artifactHashes/scope pass、resume closure closed、CompactResumeLedgerV2 queued→running→done；随后再启动真实 `repi --provider ... -p ...` provider continuation 和独立 `repi --offline --help` worker continuation，验证 env-ref-only、request-log/stdout/stderr hash、无 `.pi` 污染、无 update banner、无 literal secret，并用 same session、latest fallback、provider missing、ledger reopened、pi pollution 负例保护。`CrossSessionMultiCompactMatrixGateV1` / `npm run gate:cross-session-multi-compact-matrix` 进一步把 cross-session resume 与 MultiCompactPressureGateV1 合成同一个 closure matrix：至少八轮 compact、跨 session exact contextPath、旧 contextPath 覆盖 latest fallback、contextSha256/artifact hash 校验、multi-provider provider continuation matrix、remote provider continuation sample matrix、operator/proof-loop budget closure、CompactResumeLedgerV2 terminal row 不重开和 compact_resume_ledger_cycle_terminal_alignment 每周期 queued→running→terminal 对齐必须同时成立。
...
- Pwn advanced capability：新增 `PwnAdvancedCapabilityGateV1` / `pwn_advanced_capability_gate` / `gate:pwn-advanced-capability`，把 pwn 的 heap/tcache、format-string、SROP/ret2dlresolve、one_gadget constraint review、seccomp/sandbox syscall filter 从提示词扩展为 command pack、evidence analyzer、follow-up/self-heal、toolchain proof-exit 和测试闭环。
...
- Remote provider long-run：新增 `RemoteProviderLongRunV1` opt-in live gate，用来验证真实远程 provider 长跑但不让 CI 依赖密钥。默认 `npm run gate:remote-provider-longrun` 在无 `REPI_REMOTE_PROVIDER_LIVE=1` 时 skip/pass；显式开启 live 后读取 `REPI_REMOTE_PROVIDER_API`（`openai-completions` / `openai-responses` / `anthropic-messages`）、`REPI_REMOTE_PROVIDER_BASE_URL`、`REPI_REMOTE_PROVIDER_MODEL`、`REPI_REMOTE_PROVIDER_API_KEY_ENV` / `REPI_REMOTE_PROVIDER_API_KEY`，写 isolated `~/.repi/agent/models.json` 的 `$ENV` 引用，连续运行多轮 `repi --provider ... --model ... -p ...`，验证 marker、timeout、session/profile 隔离、env-ref-only、脱敏、无 `.pi` / update banner；其中 `openai-responses` 明确要求远程网关暴露 `POST /v1/responses`，不会静默降级到 Chat Completions；失败时写 `FailureLedgerEventV1` / `RepairQueueItemV1`，负例覆盖 missing marker、secret leak、unbounded timeout、skipped without reason、missing repair。
...
# 单独验证 CrossSessionMultiCompactMatrixGateV1 跨 session 多轮 compact closure
npm run gate:cross-session-multi-compact-matrix
...
# 单独验证 agent-dogfood 失败签名绑定 runtime manifest/claim ledger
npm run gate:agent-dogfood-failure-signature-binding
# 单独验证 agent-dogfood structured claim promotion boundary
npm run gate:agent-dogfood-structured-claims
...
- `gate:agent-dogfood-failure-signature-binding` 验证 `AgentDogfoodFailureSignatureBindingGateV1`：agent-dogfood 失败 role/synthesizer 的 runtime manifest、failure ledger、repair queue、claim ledger event 与 role-scoped dedupe window 必须共享同一个 signature/retryBudget。
- `gate:agent-dogfood-structured-claims` 验证 `AgentDogfoodStructuredClaimMergeGateV1`：agent-dogfood role/synthesizer 输出必须先变成 `StructuredClaimMergeV1` claim row；final pass 需要 runtime manifest artifact sha256、JSON query、verifierPass，narrative-only / unresolved challenge / 缺 manifest ref 不能 promotion。
- `gate:autonomous-hardening-gap-ledger` 验证 `AutonomousHardeningGapLedgerV1`：剩余 top harness 缺口必须是可执行 gap ledger，每项都有 closureGate、regressionCommands、nextCommand、artifact 和 acceptanceCriteria，不能只写 hardening_needed 文本。
- `gate:autonomous-closure-readiness` 验证 `AutonomousClosureReadinessGateV1`：每个 hardening gap 的 closureGate 必须同时存在 package script、gate script、top harness child gate、autonomy contract、docs，并能以 `--strict --no-write` 通过；`ready_for_live` 不等于 `closed`；当前 runtime rows 为 closed 且 artifact-backed，`topAutonomousDefinition=true`。
- `gate:capability-release-bundle` 验证 `CapabilityClaimReleaseBundleGateV1`：release-facing 能力声明必须有命令证据、source hash、product boundary、autonomy control plane 和 closure readiness bundle 支撑；缺证据、失败命令、secret 泄漏或 narrative-only promotion 不能进入发布口径。
- `gate:release-ci-pipeline` 验证 `ReleaseCiPipelineGateV1`：release CI 和 GitHub Actions 模板必须先跑产品边界/隔离/产品面，再跑 closure readiness 与 capability release bundle，然后跑 top harness、`npm run check` 和 no-diff guard；CI 不能要求 live provider secret。
- `gate:release-evidence-index` 验证 `ReleaseEvidenceIndexGateV1`：release 证据索引必须链接 autonomy gap ledger、closure readiness、capability bundle、CI pipeline、source hashes 和 command output hashes；hash-chain drift、缺引用、命令失败或 secret 泄漏都会阻断。
- `gate:swarm-provider-manifest-parity` 验证 `SwarmProviderManifestParityGateV1`：re_swarm manifest、worker child-session runtime 和 provider worker matrix 的 workerId/claimRefs/hash/env-ref/failure-repair refs 必须保持同源；multi-provider worker 必须共享 claim/failure/repair merge ledger，live provider-backed shared ledger windows 与 ProviderBackedLongWindowSharedMergeLedgerV1 必须覆盖所有 worker/claim/failure refs、provider/runtime manifest refs 和 env-ref-only secret handling，provider worker retry/repair rows、retry window attempt chain 与 ProviderWorkerExtendedRetryManifestChainV1 必须绑定对应 runtime manifest；all_child_sessions_match_parity_rows 还会逐 worker 校验所有 child sessions，child-session-nonfirst-row-drift 负例会阻断非首个 child session 漂移。
- `gate:worker-provider-repair-rollback-unification` 验证 `WorkerProviderRepairRollbackUnificationGateV1`：provider-worker、re_swarm worker、compound-frontier 和 operator 的 failure/repair rows 必须共享同一 signature、`RepairRollbackPolicyV1`、retry window closure 与 regression gate refs；provider-worker live repair matrix 与 RemoteProviderStateChangingRepairMatrixV1 必须覆盖多 provider state-changing repair，state lineage matrix 必须绑定 baseline snapshot、scoped mutation、restored tree hash 和 regression proof，multi-attempt / long-horizon completion chain 与 DeepCompoundProviderRepairCompletionChainV1 必须闭合同 signature regression proof；exhausted 后不能继续 unpaused rerun。
- `gate:live-conflict-arbitration-matrix` 验证 `LiveConflictArbitrationMatrixGateV1`：agent-dogfood、re_swarm、compound-frontier 和 provider-worker 的 claim 必须进入同一 conflict matrix，覆盖多 topic winner/loser、JSON-bound winner evidence、loser downgrade、provider-backed same-window multi-worker conflict table、`ProviderBackedLongWindowConflictMatrixV1` provider-backed long-window conflict matrix、`ExtendedSynthesizerTopicParseMatrixV1` long-run/extended synthesizer topic parse matrix、source runtime ledger refs，以及 orchestration success 与 platform claim success 分离。
...
- `gate:cross-session-multi-compact-matrix` 验证 `CrossSessionMultiCompactMatrixGateV1`：八轮 compact/resume 跨 session 保持 exact contextPath/hash 优先级，multi-provider 与 remote provider continuation sample matrix 必须接在 exact resume 后，operator/proof-loop closure 必须进入 done|blocked|exhausted、terminal rows 不重开，且 compact_resume_ledger_cycle_terminal_alignment 要证明每个 compact cycle 都有 queued→running→terminal 并与 resumeClosure/operatorProofClosure 一致。


### CI 自动验收模板

仓库提供 GitHub Actions 模板：`docs/reverse-agent/repi-harness.github-actions.yml`。启用时复制到 `.github/workflows/repi-harness.yml`，push / PR 会自动执行产品边界、closure readiness、capability bundle、release CI pipeline、release evidence index、top harness、repo check 和 no-diff guard：
...
npm run gate:swarm-provider-manifest-parity
...
~/.repi/agent/recon/evidence/remote/agent-parallel-dogfood/*/subagent-runtime-manifests.json  # includes failureSignatureManifestBindings
~/.repi/agent/recon/evidence/remote/agent-parallel-dogfood/*/structured-claim-merge.json
