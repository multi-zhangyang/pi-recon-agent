...
- `gate:swarm-provider-manifest-parity` 验证 `SwarmProviderManifestParityGateV1`：re_swarm manifest、worker child-session runtime 和 provider worker matrix 的 workerId/claimRefs/hash/env-ref/failure-repair refs 必须保持同源；multi-provider worker 必须共享 claim/failure/repair merge ledger，live provider-backed shared ledger windows 必须覆盖所有 worker/claim/failure refs，provider worker retry/repair rows 与 retry window attempt chain 必须绑定对应 runtime manifest。
- `gate:worker-provider-repair-rollback-unification` 验证 `WorkerProviderRepairRollbackUnificationGateV1`：provider-worker、re_swarm worker、compound-frontier 和 operator 的 failure/repair rows 必须共享同一 signature、`RepairRollbackPolicyV1`、retry window closure 与 regression gate refs；provider-worker live repair matrix 必须覆盖多 provider state-changing repair，state lineage matrix 必须绑定 baseline snapshot、scoped mutation、restored tree hash 和 regression proof，multi-attempt / long-horizon completion chain 必须闭合同 signature regression proof；exhausted 后不能继续 unpaused rerun。
