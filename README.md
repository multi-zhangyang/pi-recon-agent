...
- Memory usefulness eval：`re_memory eval` 会生成 `usefulness-eval.json`，度量 hit@1、hit@k、MRR、forbiddenLeakRate 和空召回率；`npm run gate:memory-usefulness` 用 authz / pwn 正召回、失败/跨 route forbidden memory 不进 topK、并发 append 保持 hash-chain 的 hard-eval 保护“记忆真的有用”。
...
- Memory usefulness eval hard-eval 由 `fixtures/reverse-agent/memory-usefulness.fixture.json` 与 `npm run gate:memory-usefulness` 保护：它验证 authz/pwn 场景的 hit@1、hit@k、MRR、forbidden memory 不进入 topK，并用并发 append probe 验证 lock/transaction/hash-chain 不丢写。
