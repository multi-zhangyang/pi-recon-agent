...
- `gate:autonomous-closure-readiness` 验证 `AutonomousClosureReadinessGateV1`：每个 hardening gap 的 closureGate 必须同时存在 package script、gate script、top harness child gate、autonomy contract、docs，并能以 `--strict --no-write` 通过；`ready_for_live` 不等于 `closed`，未 closed 时 `topAutonomousDefinition` 仍保持 false。
...
- `gate:cross-session-multi-compact-matrix` 验证 `CrossSessionMultiCompactMatrixGateV1`：五轮 compact/resume 跨 session 保持 exact contextPath/hash 优先级，multi-provider 与 remote provider continuation sample matrix 必须接在 exact resume 后，operator/proof-loop closure 必须进入 done|blocked|exhausted、terminal rows 不重开，且 compact_resume_ledger_cycle_terminal_alignment 要证明每个 compact cycle 都有 queued→running→terminal 并与 resumeClosure/operatorProofClosure 一致。
