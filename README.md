...
- `re_memory events/search-events/consolidate/distill`：读取结构化长期记忆、检索可复用 case、汇总高质量经验，并把事件蒸馏为可注入的 command template / verifier rule / worker routing hint。
...
当前 REPI 的长期记忆分三层：原始事件、case 聚合、蒸馏 pattern/quarantine。
