...
function staticContractChecks() {
	...
	checks.push(markerCheck("npm:top-harness-script", "package.json", ["gate:repi-harness", "gate:repi-product", "gate:repi-isolation", "gate:repi-product-surface", "gate:context-compact", "gate:compact-resume-chain", "gate:context-runtime-schema", "gate:memory-contract", "gate:memory-utility", "gate:memory-feedback", "gate:memory-hybrid", "gate:memory-usefulness", "gate:memory-distiller", "gate:memory-sedimentation", "gate:memory-store", "gate:memory-swarm-writeback", "gate:worker-runtime-pool", "gate:worker-child-session", "gate:structured-claim-merge", "install:repi", "clean:repi-legacy-profile"], ["install:recon-pi", "gate:pi-recon-primary"]));
