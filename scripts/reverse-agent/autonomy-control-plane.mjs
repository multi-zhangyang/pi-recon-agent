...
const REQUIREMENTS = [
	...
	},
	{
		...
		recommendedWork: [
			...
			"MultiCompactPressureGateV1 已覆盖 re_operator/re_proof_loop 反写与 multi-compact 负例；继续扩大更多 latest artifact consumer 的 scope verdict propagation。",
	...
	},
	{
		...
		hardeningNeeded: [
			...
			"StructuredClaimMergeV1 已接入 bounded re_swarm live gate；AgentDogfoodStructuredClaimMergeGateV1 已把 agent-dogfood role/synthesizer summary 降为 structured claim rows，final promotion 必须有 runtime manifest sha256、JSON query 和 verifierPass；后续继续扩大真实 multi-worker 冲突样本。",
		...
		recommendedWork: [
			...
			"继续把同一 ledger/schema 接入通用 re_swarm 独立 sub-agent/session runtime，并扩大 AgentDogfoodStructuredClaimMergeGateV1 / StructuredClaimMergeV1 claim-aware merge 的冲突仲裁样本。",
