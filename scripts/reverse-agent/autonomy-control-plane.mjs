...
const CONTROL_CONTRACTS = [
	...
	},
	{
		...
		description: "同一个矩阵必须证明多轮 compact/resume 跨 session 精确恢复、旧 contextPath 优先 latest fallback、contextSha256/artifact hash 校验、provider continuation、operator/proof-loop budget closure 和 CompactResumeLedgerV2 terminal row 不重开。",
		requiredFields: [
			"kind",
			"schemaVersion",
			"generatedAt",
		...
		nestedRequired: {
			matrix: ["kind", "schemaVersion", "closureGate", "sessions", "compactCycles", "providerContinuations", "operatorProofClosures", "compactResumeLedger"],
			...
			providerContinuations: ["cycleId", "resumeSessionId", "providerName", "modelId", "apiKeyEnvRefOnly", "requestLogPath", "stdoutPath", "stderrPath"],
	...
	},
	{
		...
		description: "provider-worker、re_swarm worker、compound-frontier 与 operator repair 必须共享 FailureLedgerEventV1/RepairQueueItemV1 signature、RepairRollbackPolicyV1、retry window closure 与 regression gate refs。",
		requiredFields: [
			"kind",
			"schemaVersion",
			"generatedAt",
		...
		nestedRequired: {
			unificationReport: ["kind", "schemaVersion", "closureGate", "scenarios", "signatureIndex", "promotionPolicy"],
	...
	},
	{
		...
		normalChecks: [
			...
			},
			{
				...
				description: "CrossSessionMultiCompactMatrixGateV1 把 CrossSessionResumeLiveV1 与 MultiCompactPressureGateV1 组合成同一矩阵，验证多轮 compact、跨 session exact contextPath、provider continuation 和 operator/proof-loop closure。",
				files: ["scripts/reverse-agent/cross-session-multi-compact-matrix-gate.mjs"],
				markers: ["CrossSessionMultiCompactMatrixGateV1", "runtime:cross-session-same-run", "runtime:old-context-path-over-latest-after-multiple-compacts", "runtime:provider-continuation-after-exact-resume", "fixture:negative-rejections"],
			},
			{
				...
				markers: ["CrossSessionMultiCompactMatrixGateV1", "cross_session_multi_compact_same_run", "provider_continuation_after_exact_resume", "terminal_resume_rows_not_reopened"],
			},
			{
				id: "cross_session_multi_compact_matrix_fixture",
				description: "Cross-session multi-compact fixture 覆盖 latest fallback、context sha drift、artifact drift、provider missing、budget open、terminal reopen、same session 和 ledger drift 负例。",
				files: ["fixtures/reverse-agent/cross-session-multi-compact-matrix.fixture.json"],
				markers: ["repi-cross-session-multi-compact-matrix-fixture", "latest-fallback-without-explicit-context", "provider-continuation-missing", "terminal-row-reopened"],
			},
			{
		...
		recommendedWork: [
			...
			"CrossSessionMultiCompactMatrixGateV1 已把 MultiCompactPressureGateV1 与 CrossSessionResumeLiveV1 合并成同一 closure gate，覆盖跨 session、多轮 compact、provider continuation、operator/proof-loop closure；继续扩大更多 latest artifact consumer 的 scope verdict propagation。",
			"保持 cross-session multi-compact matrix 与 ContextPackV2 / ResumeContractV2 / CompactResumeLedgerV2 runtime markers 同步，并继续扩展 provider continuation live 样本。",
		],
	},
	{
		...
		normalChecks: [
			...
			},
			{
				...
				description: "WorkerProviderRepairRollbackUnificationGateV1 把 provider-worker、re_swarm worker、compound-frontier 和 operator 的 repair rows 统一到同一 signature/rollback/regression 合同，阻断 exhausted 后 unpaused rerun。",
				files: ["scripts/reverse-agent/worker-provider-repair-rollback-unification-gate.mjs"],
				markers: ["WorkerProviderRepairRollbackUnificationGateV1", "runtime:provider-worker-state-change-rollback-policy", "runtime:exhausted-blocks-unpaused-rerun", "provider_worker_refs_preserve_manifest_request_log_rollback", "policy-failure-repair-unlinked"],
			},
			{
				id: "worker_provider_repair_rollback_unification_fixture",
				description: "WorkerProviderRepairRollbackUnification fixture 覆盖 provider-worker、swarm-worker、compound-frontier、operator 和 signature/rollback/repair 负例。",
				files: ["fixtures/reverse-agent/worker-provider-repair-rollback-unification.fixture.json"],
				markers: ["repi-worker-provider-repair-rollback-unification-fixture", "provider-worker-state-change", "swarm-worker-provider-repair", "signature-mismatch", "policy-failure-repair-unlinked"],
			},
			{
				id: "worker_provider_repair_rollback_unification_schema",
				description: "WorkerProviderRepairRollbackUnification schema 固化同 signature、rollback policy、provider worker refs、retry window 和 regression gate refs。",
				files: ["schemas/reverse-agent/worker-provider-repair-rollback-unification.schema.json"],
				markers: ["WorkerProviderRepairRollbackUnificationGateV1", "same_signature_failure_repair_rollback_regression", "provider_worker_state_change_writes_rollback_policy", "regression_gate_refs_match_repair_queue"],
			},
			{
...
const HARDENING_GAP_CATALOG = [
	...
	},
	{
		...
		currentEvidence: ["gate:cross-session-multi-compact-matrix", "gate:multi-compact-pressure", "gate:cross-session-resume-live", "gate:context-runtime-schema"],
		missingRuntimeProof: ["broader real provider continuation samples across remote models", "longer cross-session compaction chains beyond bounded local matrix"],
	...
	},
	{
		...
		currentEvidence: ["gate:worker-provider-repair-rollback-unification", "gate:provider-failure-injection", "gate:repair-rollback-policy", "gate:agent-dogfood-failure-signature-binding"],
		missingRuntimeProof: ["broader live provider-worker state-changing repairs beyond bounded local fixtures", "longer compound/provider repair completion across real retry windows"],
