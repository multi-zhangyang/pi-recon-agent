...
const CONTROL_CONTRACTS = [
	...
	},
	{
		...
		description: "同一个矩阵必须证明至少三轮 compact/resume 跨 session 精确恢复、旧 contextPath 优先 latest fallback、contextSha256/artifact hash 校验、multi-provider provider continuation、operator/proof-loop budget closure 和 CompactResumeLedgerV2 terminal row 不重开。",
		requiredFields: [
			"kind",
			"schemaVersion",
			"generatedAt",
		...
		nestedRequired: {
			matrix: ["kind", "schemaVersion", "closureGate", "sessions", "compactCycles", "providerContinuations", "providerContinuationMatrix", "operatorProofClosures", "compactResumeLedger"],
	...
	},
	{
		...
		description: "provider-worker、re_swarm worker、compound-frontier 与 operator repair 必须共享 FailureLedgerEventV1/RepairQueueItemV1 signature、RepairRollbackPolicyV1、retry window closure、regression gate refs、provider-worker live repair matrix、state lineage snapshot matrix 与 long-horizon completion chain。",
		requiredFields: [
			"kind",
			"schemaVersion",
			"generatedAt",
		...
		nestedRequired: {
			unificationReport: ["kind", "schemaVersion", "closureGate", "scenarios", "liveRepairMatrix", "retryWindowCompletionChain", "stateLineageSnapshotMatrix", "longHorizonRepairCompletionChain", "signatureIndex", "promotionPolicy"],
		...
		enumFields: {
			...
			"scenarios.id": ["provider-worker-state-change", "swarm-worker-provider-repair", "provider-worker-cache-state-repair", "swarm-worker-tool-state-repair", "compound-frontier-retry-window", "compound-provider-long-horizon-repair", "operator-exhausted-escalation"],
			"negativeCases.id": [
				"signature-mismatch",
	...
	},
	{
		...
		description: "re_swarm command-level manifests、WorkerChildSessionRuntimeBatchV1 和 ParallelProviderWorkerMatrixV1 必须在 workerId、claimRefs、hash、provider env-ref、failure/repair refs 上保持同源，并覆盖 live provider-backed shared ledger matrix 与 retry window manifest binding chain。",
		requiredFields: [
			"kind",
			"schemaVersion",
		...
		nestedRequired: {
			parityReport: ["kind", "schemaVersion", "source", "parityRows", "promotionPolicy", "closureGate", "sharedMergeLedger", "retryRepairBindings", "liveProviderBackedSharedLedgerMatrix", "retryWindowManifestBindingChain"],
		...
		enumFields: {
			...
			"negativeCases.id": ["worker-id-mismatch", "claim-ref-dropped", "missing-runtime-hash", "literal-provider-secret", "failure-repair-unlinked", "single-provider-matrix", "shared-ledger-worker-missing", "retry-repair-manifest-unbound", "shared-ledger-window-provider-missing", "retry-window-nonmonotonic", "retry-window-manifest-drift"],
		},
		invariants: [
...
const REQUIREMENTS = [
	{
		...
		normalChecks: [
			...
			},
			{
				...
				description: "SwarmProviderManifestParityGateV1 将 re_swarm SubagentRuntimeManifestV1、WorkerChildSessionRuntimeBatchV1 与 ParallelProviderWorkerMatrixV1 的 workerId/claimRefs/hash/env-ref/failure-repair refs 做同源校验，并覆盖 live provider-backed shared ledger matrix 与 retry window manifest binding chain。",
				...
				markers: ["SwarmProviderManifestParityGateV1", "fixture:positive-parity", "fixture:negative-parity", "liveProviderBackedSharedLedgerMatrix", "retryWindowManifestBindingChain", "worker-id-mismatch", "literal-provider-secret", "failure-repair-unlinked", "multi_provider_workers_share_claim_failure_merge_ledger", "provider_worker_retry_repair_rows_bound_to_worker_manifest", "live_provider_backed_multi_provider_shared_ledger_matrix", "provider_worker_retry_window_manifest_binding_chain", "single-provider-matrix", "retry-repair-manifest-unbound", "shared-ledger-window-provider-missing", "retry-window-manifest-drift"],
			},
			{
	...
	},
	{
		...
		normalChecks: [
			...
			},
			{
				...
				description: "CrossSessionMultiCompactMatrixGateV1 把 CrossSessionResumeLiveV1 与 MultiCompactPressureGateV1 组合成同一矩阵，验证三轮 compact、跨 session exact contextPath、multi-provider provider continuation 和 operator/proof-loop closure。",
				files: ["scripts/reverse-agent/cross-session-multi-compact-matrix-gate.mjs"],
				markers: ["CrossSessionMultiCompactMatrixGateV1", "runtime:cross-session-same-run", "runtime:old-context-path-over-latest-after-multiple-compacts", "runtime:provider-continuation-after-exact-resume", "runtime:provider-continuation-matrix-multi-provider", "runtime:longer-cross-session-compaction-chain", "fixture:negative-rejections"],
			},
			{
				id: "cross_session_multi_compact_matrix_schema",
				description: "Cross-session multi-compact matrix schema 固化 exact contextPath、provider continuation、operator/proof closure 和 CompactResumeLedgerV2 terminal row 不变量。",
				files: ["schemas/reverse-agent/cross-session-multi-compact-matrix.schema.json"],
				markers: ["CrossSessionMultiCompactMatrixGateV1", "cross_session_multi_compact_same_run", "provider_continuation_after_exact_resume", "provider_continuation_matrix_multi_provider", "longer_cross_session_compaction_chain", "ProviderContinuationMatrixV1", "terminal_resume_rows_not_reopened"],
			},
			{
				id: "cross_session_multi_compact_matrix_fixture",
				description: "Cross-session multi-compact fixture 覆盖 latest fallback、context sha drift、artifact drift、provider missing、budget open、terminal reopen、same session、ledger drift、single provider 和 chain too short 负例。",
				files: ["fixtures/reverse-agent/cross-session-multi-compact-matrix.fixture.json"],
				markers: ["repi-cross-session-multi-compact-matrix-fixture", "latest-fallback-without-explicit-context", "provider-continuation-missing", "terminal-row-reopened", "provider-continuation-single-provider", "compact-chain-too-short"],
			},
			{
		...
		recommendedWork: [
			...
			"CrossSessionMultiCompactMatrixGateV1 已把 MultiCompactPressureGateV1 与 CrossSessionResumeLiveV1 合并成同一 closure gate，覆盖跨 session、三轮 compact、multi-provider provider continuation、operator/proof-loop closure；继续扩大更多 latest artifact consumer 的 scope verdict propagation。",
	...
	},
	{
		...
		normalChecks: [
			...
			},
			{
				...
				description: "WorkerProviderRepairRollbackUnificationGateV1 把 provider-worker、re_swarm worker、compound-frontier 和 operator 的 repair rows 统一到同一 signature/rollback/regression 合同，补 provider-worker live repair matrix、state lineage snapshot matrix 与 long-horizon completion chain，阻断 exhausted 后 unpaused rerun。",
				files: ["scripts/reverse-agent/worker-provider-repair-rollback-unification-gate.mjs"],
				markers: ["WorkerProviderRepairRollbackUnificationGateV1", "runtime:provider-worker-state-change-rollback-policy", "runtime:provider-worker-live-state-change-repair-matrix", "runtime:multi-attempt-retry-window-completion-chain", "runtime:provider-worker-state-lineage-snapshot-matrix", "runtime:compound-provider-long-horizon-repair-completion-chain", "runtime:exhausted-blocks-unpaused-rerun", "provider_worker_refs_preserve_manifest_request_log_rollback", "provider_worker_state_lineage_snapshot_matrix", "compound_provider_long_horizon_repair_completion_chain", "policy-failure-repair-unlinked"],
			},
			{
				id: "worker_provider_repair_rollback_unification_fixture",
				description: "WorkerProviderRepairRollbackUnification fixture 覆盖 provider-worker、swarm-worker、compound-frontier、operator、live repair matrix、multi-attempt chain、state lineage、long-horizon chain 和 signature/rollback/repair 负例。",
				files: ["fixtures/reverse-agent/worker-provider-repair-rollback-unification.fixture.json"],
				markers: ["repi-worker-provider-repair-rollback-unification-fixture", "provider-worker-state-change", "swarm-worker-provider-repair", "provider-worker-cache-state-repair", "compound-provider-long-horizon-repair", "signature-mismatch", "policy-failure-repair-unlinked", "live-repair-matrix-missing-provider", "retry-window-not-monotonic", "state-lineage-missing-baseline", "long-horizon-signature-drift"],
			},
			{
				id: "worker_provider_repair_rollback_unification_schema",
				description: "WorkerProviderRepairRollbackUnification schema 固化同 signature、rollback policy、provider worker refs、retry window、live repair matrix、multi-attempt completion chain、state lineage snapshot matrix、long-horizon completion chain 和 regression gate refs。",
				files: ["schemas/reverse-agent/worker-provider-repair-rollback-unification.schema.json"],
				markers: ["WorkerProviderRepairRollbackUnificationGateV1", "same_signature_failure_repair_rollback_regression", "provider_worker_state_change_writes_rollback_policy", "ProviderWorkerLiveRepairMatrixV1", "MultiAttemptRetryWindowCompletionChainV1", "ProviderWorkerStateLineageSnapshotMatrixV1", "CompoundProviderLongHorizonRepairCompletionChainV1", "regression_gate_refs_match_repair_queue"],
			},
			{
		...
		hardeningNeeded: [
			"Provider failure injection 已接入 strict failure/repair validator；RepairRollbackPolicyV1 已把 baseline/allowlist/regression/rollback 接成独立 hard-eval；AgentDogfoodFailureSignatureBindingGateV1 已把 agent-dogfood subagent runtime manifest 与 failure signature / retryBudget / dedupeWindow 绑定；WorkerProviderRepairRollbackUnificationGateV1 已把 provider-worker、re_swarm worker、compound-frontier 和 operator repair rows 统一到同一 signature/rollback/regression 合同，并补 provider-worker live repair matrix、state lineage snapshot matrix 与 long-horizon retry window completion chain；后续继续扩大真实 provider-worker state-changing repair 样本。",
			"所有 runtime retry 继续复用同一 signature 和 budget/retryBudget；达到 exhausted 后停止盲 retry，转 repair/escalate。",
			"保持 WorkerProviderRepairRollbackUnificationGateV1 作为 provider/worker repair closure gate，并继续把更多真实 state-changing repair 纳入 live regression、state lineage 和 long-horizon completion chain。",
		],
		recommendedWork: [
...
const HARDENING_GAP_CATALOG = [
	{
		...
		currentEvidence: ["gate:swarm-provider-manifest-parity", "gate:worker-child-session", "gate:parallel-provider-worker-matrix", "gate:worker-lease-scheduler", "SwarmProviderSharedMergeLedgerV1", "SwarmProviderRetryRepairBindingV1", "LiveProviderBackedSharedLedgerMatrixV1", "ProviderWorkerRetryWindowManifestBindingChainV1"],
		missingRuntimeProof: ["broader real remote provider-backed shared merge ledger windows beyond bounded live matrix", "longer provider worker retry/repair manifest binding across real remote retry windows beyond bounded chain"],
	...
	},
	{
		...
		currentEvidence: ["gate:cross-session-multi-compact-matrix", "gate:multi-compact-pressure", "gate:cross-session-resume-live", "gate:context-runtime-schema", "ProviderContinuationMatrixV1", "longer_cross_session_compaction_chain"],
		missingRuntimeProof: ["broader real remote provider continuation samples beyond bounded multi-provider matrix", "longer cross-session compaction chains across live sessions beyond three-cycle bounded matrix"],
	...
	},
	{
		...
		currentEvidence: ["gate:worker-provider-repair-rollback-unification", "gate:provider-failure-injection", "gate:repair-rollback-policy", "gate:agent-dogfood-failure-signature-binding", "ProviderWorkerLiveRepairMatrixV1", "MultiAttemptRetryWindowCompletionChainV1", "ProviderWorkerStateLineageSnapshotMatrixV1", "CompoundProviderLongHorizonRepairCompletionChainV1"],
		missingRuntimeProof: ["broader real remote provider-worker state-changing repairs beyond bounded state lineage matrix", "longer compound/provider repair completion across real retry windows beyond bounded long-horizon chain"],
