...
const CONTROL_CONTRACTS = [
	...
	},
	{
		...
		description: "provider-worker、re_swarm worker、compound-frontier 与 operator repair 必须共享 FailureLedgerEventV1/RepairQueueItemV1 signature、RepairRollbackPolicyV1、retry window closure、regression gate refs、provider-worker live repair matrix 与 multi-attempt completion chain。",
		requiredFields: [
			"kind",
			"schemaVersion",
			"generatedAt",
		...
		nestedRequired: {
			unificationReport: ["kind", "schemaVersion", "closureGate", "scenarios", "liveRepairMatrix", "retryWindowCompletionChain", "signatureIndex", "promotionPolicy"],
		...
		enumFields: {
			...
			"scenarios.id": ["provider-worker-state-change", "swarm-worker-provider-repair", "compound-frontier-retry-window", "operator-exhausted-escalation"],
			"negativeCases.id": [
				"signature-mismatch",
	...
	},
	{
		...
		description: "re_swarm command-level manifests、WorkerChildSessionRuntimeBatchV1 和 ParallelProviderWorkerMatrixV1 必须在 workerId、claimRefs、hash、provider env-ref、failure/repair refs 上保持同源。",
		requiredFields: [
			"kind",
			"schemaVersion",
		...
		nestedRequired: {
			parityReport: ["kind", "schemaVersion", "source", "parityRows", "promotionPolicy", "closureGate"],
			...
			parityChecks: ["workerIdMatch", "claimRefsPreserved", "hashesPresent", "providerEnvRefsOnly", "failureRepairLinked"],
			negativeCases: ["id", "mutates", "expect", "mustNotPromote"],
		},
		enumFields: {
			requiredGates: [
			...
			"negativeCases.id": ["worker-id-mismatch", "claim-ref-dropped", "missing-runtime-hash", "literal-provider-secret", "failure-repair-unlinked"],
		},
		invariants: [
...
const REQUIREMENTS = [
	{
		...
		hardeningNeeded: [
			...
			"继续让 SwarmProviderManifestParityGateV1 从 bounded parity 扩展到更多真实 worker/provider 场景。",
		],
		recommendedWork: [
	...
	},
	{
		...
		normalChecks: [
			...
			},
			{
				...
				description: "WorkerProviderRepairRollbackUnificationGateV1 把 provider-worker、re_swarm worker、compound-frontier 和 operator 的 repair rows 统一到同一 signature/rollback/regression 合同，补 provider-worker live repair matrix 与 multi-attempt completion chain，阻断 exhausted 后 unpaused rerun。",
				files: ["scripts/reverse-agent/worker-provider-repair-rollback-unification-gate.mjs"],
				markers: ["WorkerProviderRepairRollbackUnificationGateV1", "runtime:provider-worker-state-change-rollback-policy", "runtime:provider-worker-live-state-change-repair-matrix", "runtime:multi-attempt-retry-window-completion-chain", "runtime:exhausted-blocks-unpaused-rerun", "provider_worker_refs_preserve_manifest_request_log_rollback", "policy-failure-repair-unlinked"],
			},
			{
				id: "worker_provider_repair_rollback_unification_fixture",
				description: "WorkerProviderRepairRollbackUnification fixture 覆盖 provider-worker、swarm-worker、compound-frontier、operator、live repair matrix、multi-attempt chain 和 signature/rollback/repair 负例。",
				files: ["fixtures/reverse-agent/worker-provider-repair-rollback-unification.fixture.json"],
				markers: ["repi-worker-provider-repair-rollback-unification-fixture", "provider-worker-state-change", "swarm-worker-provider-repair", "signature-mismatch", "policy-failure-repair-unlinked", "live-repair-matrix-missing-provider", "retry-window-not-monotonic"],
			},
			{
				id: "worker_provider_repair_rollback_unification_schema",
				description: "WorkerProviderRepairRollbackUnification schema 固化同 signature、rollback policy、provider worker refs、retry window、live repair matrix、multi-attempt completion chain 和 regression gate refs。",
				files: ["schemas/reverse-agent/worker-provider-repair-rollback-unification.schema.json"],
				markers: ["WorkerProviderRepairRollbackUnificationGateV1", "same_signature_failure_repair_rollback_regression", "provider_worker_state_change_writes_rollback_policy", "ProviderWorkerLiveRepairMatrixV1", "MultiAttemptRetryWindowCompletionChainV1", "regression_gate_refs_match_repair_queue"],
			},
			{
...
const HARDENING_GAP_CATALOG = [
	...
	},
	{
		...
		currentEvidence: ["gate:worker-provider-repair-rollback-unification", "gate:provider-failure-injection", "gate:repair-rollback-policy", "gate:agent-dogfood-failure-signature-binding", "ProviderWorkerLiveRepairMatrixV1", "MultiAttemptRetryWindowCompletionChainV1"],
		missingRuntimeProof: ["broader live provider-worker state-changing repairs beyond bounded live repair matrix", "longer compound/provider repair completion across real retry windows beyond three-attempt bounded completion chain"],
