...
const CONTROL_CONTRACTS = [
	...
	},
	{
		...
		description: "同一个矩阵必须证明至少八轮 compact/resume 跨 session 精确恢复、旧 contextPath 优先 latest fallback、contextSha256/artifact hash 校验、multi-provider 与 remote provider continuation sample matrix、operator/proof-loop budget closure、CompactResumeLedgerV2 terminal row 不重开和 compact_resume_ledger_cycle_terminal_alignment 每周期 terminal 对齐。",
		requiredFields: [
			"kind",
			"schemaVersion",
			"generatedAt",
		...
		nestedRequired: {
			...
			compactResumeLedger: ["kind", "transitionPath", "appendOnly", "hashChainOk", "terminalRowsNotReopened", "cycleTerminalAlignmentOk", "ledgerCycleAlignment", "transitions"],
...
const REQUIREMENTS = [
	{
		...
		normalChecks: [
			...
			{
				id: "toolchain_domain_capability_gate",
				description: "ToolchainDomainCapabilityV1 把专业逆向/渗透工具链能力做成 runtime:toolchain-doctor 与 domain_toolchain_matrix：按 web-api、rev-native、pwn、mobile、pcap-dfir、firmware-iot、crypto、cloud-identity、exploit-reliability 分类输出 present/missing/fallback/proof-exit/next commands，避免能力退化成泛化建议。",
				files: ["packages/coding-agent/src/core/recon-profile.ts", "scripts/reverse-agent/toolchain-domain-capability-gate.mjs", "schemas/reverse-agent/toolchain-domain-capability.schema.json"],
				markers: [
			...
			{
				id: "toolchain_domain_capability_npm_gate",
				description: "package 暴露 gate:toolchain-domain-capability，供顶级 harness 和 CI 验证工具链/领域能力不是文档口号。",
				files: ["package.json"],
				markers: ["gate:toolchain-domain-capability", "toolchain-domain-capability-gate.mjs"],
			},
			{
				id: "toolchain_domain_capability_top_harness_gate",
				description: "top harness 把 ToolchainDomainCapabilityV1 接成 child gate，防止专业工具链层游离于 release harness 外。",
			...
			{
				id: "professional_runtime_bridge_gate",
				description: "ProfessionalRuntimeBridgesGateV1 把真实工具链桥接、自动利用验证、Web/CDP replay 与 Frida/Mobile 动态桥接接成 runtime_execution_bridge_matrix：每条桥必须有可执行命令模板、fallback 工具、env-ref-only 配置、artifact plan、proof-exit 和负例拒绝，避免专业能力停留在叙述层。",
				files: ["packages/coding-agent/src/core/recon-profile.ts", "repi-profile/extensions/reverse-pentest-core.ts", "scripts/reverse-agent/professional-runtime-bridges-gate.mjs", "schemas/reverse-agent/professional-runtime-bridges.schema.json"],
				markers: [
			...
			{
				id: "runtime_adapter_execution_gate",
				description: "RuntimeAdapterExecutionGateV1 把 r2/Ghidra/Frida/CDP/pwntools/tshark/binwalk 风格 adapter 做成 runner → parser → artifact ingest 的闭环：每个 adapter 必须有 native/fallback runner、parser rules、artifact kinds、evidence-ledger/knowledge-graph/memory-event 入库目标、proof-exit signals 和 env-ref-only 配置。",
				files: ["packages/coding-agent/src/core/recon-profile.ts", "repi-profile/extensions/reverse-pentest-core.ts", "scripts/reverse-agent/runtime-adapter-execution-gate.mjs", "schemas/reverse-agent/runtime-adapter-execution.schema.json"],
				markers: [
			...
			{
				id: "runtime_adapter_execution_npm_gate",
				description: "package 暴露 gate:runtime-adapter-execution，供顶级 harness 和 CI 验证 adapter runner/parser/ingest 不是文档口号。",
				files: ["package.json"],
				markers: ["gate:runtime-adapter-execution", "runtime-adapter-execution-gate.mjs"],
			},
			{
				id: "runtime_adapter_execution_top_harness_gate",
				description: "top harness 把 RuntimeAdapterExecutionGateV1 接成 child gate，防止真实 adapter 执行层游离于 release harness 外。",
				files: ["scripts/reverse-agent/repi-top-harness.mjs"],
				markers: ["gate:runtime-adapter-execution", "toolchain:runtime-adapter-execution-hard-eval", "RuntimeAdapterExecutionGateV1", "child:gate:runtime-adapter-execution"],
			},
			{
				id: "professional_runtime_bridge_npm_gate",
				description: "package 暴露 gate:professional-runtime-bridges，供顶级 harness 和 CI 验证真实工具桥接、利用验证、CDP replay、Frida/Mobile 动态桥接不是文档口号。",
				files: ["package.json"],
				markers: ["gate:professional-runtime-bridges", "professional-runtime-bridges-gate.mjs"],
			},
			{
				id: "professional_runtime_bridge_top_harness_gate",
				description: "top harness 把 ProfessionalRuntimeBridgesGateV1 接成 child gate，防止工具桥接/验证/replay/mobile 动态分析层游离于 release harness 外。",
				files: ["scripts/reverse-agent/repi-top-harness.mjs"],
				markers: ["gate:professional-runtime-bridges", "toolchain:professional-runtime-bridges-hard-eval", "ProfessionalRuntimeBridgesGateV1", "child:gate:professional-runtime-bridges"],
			},
			{
			...
			{
				id: "relane_specialist_command_pack_gate",
				description: "ReLaneSpecialistCommandPackGateV1 把每个专业域的 route → lane seeds → command pack markers → analyzer anchors → self-heal commands → proof-exit bridge 做成可验证闭环，防止 re_lane plan 退化成泛化建议。",
				files: ["packages/coding-agent/src/core/recon-profile.ts", "scripts/reverse-agent/relane-specialist-command-pack-gate.mjs", "schemas/reverse-agent/relane-specialist-command-pack.schema.json"],
				markers: ["ReLaneSpecialistCommandPackGateV1", "runtime:re_lane-specialist-command-pack", "route_to_domain_lane_seed_matrix", "domain_lane_command_pack_markers", "specialist_evidence_analyzer_anchor_matrix", "self_heal_command_fallback_matrix", "proof_exit_bridge_matrix", "re_lane_specialist_pack"],
			},
			{
				id: "relane_specialist_command_pack_npm_gate",
				description: "package 暴露 gate:relane-specialist-command-pack，供 top harness/CI 验证专项 command pack 闭环。",
				files: ["package.json"],
				markers: ["gate:relane-specialist-command-pack", "relane-specialist-command-pack-gate.mjs"],
			},
			{
				id: "relane_specialist_command_pack_top_harness_gate",
				description: "top harness 把 ReLaneSpecialistCommandPackGateV1 接成 child gate，防止专业域 command pack/analyzer/self-heal 只存在于源码注释。",
				files: ["scripts/reverse-agent/repi-top-harness.mjs"],
				markers: ["gate:relane-specialist-command-pack", "toolchain:relane-specialist-command-pack-hard-eval", "ReLaneSpecialistCommandPackGateV1", "child:gate:relane-specialist-command-pack"],
			},
			{
	...
	},
	{
		...
		normalChecks: [
			...
			},
			{
				...
				description: "CrossSessionMultiCompactMatrixGateV1 把 CrossSessionResumeLiveV1 与 MultiCompactPressureGateV1 组合成同一矩阵，验证八轮 compact、跨 session exact contextPath、multi-provider 与 remote provider continuation sample matrix、operator/proof-loop closure 和 compact_resume_ledger_cycle_terminal_alignment。",
				files: ["scripts/reverse-agent/cross-session-multi-compact-matrix-gate.mjs"],
				markers: ["CrossSessionMultiCompactMatrixGateV1", "runtime:cross-session-same-run", "runtime:old-context-path-over-latest-after-multiple-compacts", "runtime:provider-continuation-after-exact-resume", "runtime:provider-continuation-matrix-multi-provider", "runtime:five-cycle-cross-session-compaction-chain", "runtime:remote-provider-continuation-sample-matrix", "runtime:longer-cross-session-compaction-chain", "runtime:compact-resume-ledger-cycle-terminal-alignment", "fixture:negative-rejections"],
			},
			{
				id: "cross_session_multi_compact_matrix_schema",
				description: "Cross-session multi-compact matrix schema 固化 exact contextPath、provider continuation、remote provider sample matrix、operator/proof closure 和 CompactResumeLedgerV2 terminal row 和 compact_resume_ledger_cycle_terminal_alignment 不变量。",
			...
			},
			{
				...
				description: "Cross-session multi-compact fixture 覆盖 latest fallback、context sha drift、artifact drift、provider missing、budget open、terminal reopen、same session、ledger drift、single provider、chain too short、remote provider sample missing、ledger-terminal-missing-after-rehash 和 secret leak 负例。",
				files: ["fixtures/reverse-agent/cross-session-multi-compact-matrix.fixture.json"],
				markers: ["repi-cross-session-multi-compact-matrix-fixture", "latest-fallback-without-explicit-context", "provider-continuation-missing", "terminal-row-reopened", "provider-continuation-single-provider", "compact-chain-too-short", "five-cycle-chain-too-short", "ledger-terminal-missing-after-rehash", "remote-provider-secret-leak"],
			},
			{
		...
		recommendedWork: [
			...
			"CrossSessionMultiCompactMatrixGateV1 已把 MultiCompactPressureGateV1 与 CrossSessionResumeLiveV1 合并成同一 closure gate，覆盖跨 session、八轮 compact、multi-provider 与 remote provider continuation sample matrix、operator/proof-loop closure；继续扩大更多 latest artifact consumer 的 scope verdict propagation。",
...
const HARDENING_GAP_CATALOG = [
	{
		...
		acceptanceCriteria: [
			...
			"ProviderWorkerExtendedRetryManifestChainV1 covers at least ten retry attempts across at least two providers with same-signature manifest binding and terminal status proof",
	...
	},
	{
		...
		currentEvidence: ["gate:cross-session-multi-compact-matrix", "gate:multi-compact-pressure", "gate:cross-session-resume-live", "gate:context-runtime-schema", "ProviderContinuationMatrixV1", "RemoteProviderContinuationSampleMatrixV1", "longer_cross_session_compaction_chain", "five_cycle_cross_session_compaction_chain", "eight_cycle_cross_session_compaction_chain", "remote_provider_continuation_eight_sample_matrix", "compact_resume_ledger_cycle_terminal_alignment", "ledger-terminal-missing-after-rehash"],
		missingRuntimeProof: [],
		artifactBackedClosure: true,
		closureEvidenceRefs: ["scripts/reverse-agent/cross-session-multi-compact-matrix-gate.mjs", "fixtures/reverse-agent/cross-session-multi-compact-matrix.fixture.json", "eight_cycle_cross_session_compaction_chain", "remote_provider_continuation_eight_sample_matrix"],
		...
		acceptanceCriteria: [
			...
			"compact_resume_ledger_cycle_terminal_alignment proves every compact cycle has queued→running→terminal, terminal state matches resumeClosure/operatorProofClosure, context hash matches, and no orphan transitions",
	...
	},
	{
		...
		currentEvidence: ["gate:worker-provider-repair-rollback-unification", "gate:provider-failure-injection", "gate:repair-rollback-policy", "gate:agent-dogfood-failure-signature-binding", "ProviderWorkerLiveRepairMatrixV1", "MultiAttemptRetryWindowCompletionChainV1", "ProviderWorkerStateLineageSnapshotMatrixV1", "CompoundProviderLongHorizonRepairCompletionChainV1", "RemoteProviderStateChangingRepairMatrixV1", "DeepCompoundProviderRepairCompletionChainV1", "remote_provider_eight_state_changing_repair_matrix", "deep_compound_provider_ten_attempt_repair_chain"],
		missingRuntimeProof: [],
		artifactBackedClosure: true,
		closureEvidenceRefs: ["scripts/reverse-agent/worker-provider-repair-rollback-unification-gate.mjs", "fixtures/reverse-agent/worker-provider-repair-rollback-unification.fixture.json", "remote_provider_eight_state_changing_repair_matrix", "deep_compound_provider_ten_attempt_repair_chain"],
		...
		acceptanceCriteria: [
			...
			"DeepCompoundProviderRepairCompletionChainV1 covers at least ten same-signature attempts and twenty total attempt rows with per-attempt runtime refs and regression proof",
		],
	},
	{
		...
		currentEvidence: ["gate:live-conflict-arbitration-matrix", "gate:structured-claim-merge", "gate:agent-dogfood-structured-claims", "gate:runtime-ledger-quality", "ProviderBackedLongWindowConflictMatrixV1", "ExtendedSynthesizerTopicParseMatrixV1", "provider_backed_eight_window_conflict_matrix", "synthesizer_extended_ten_topic_parse_matrix"],
		missingRuntimeProof: [],
		artifactBackedClosure: true,
		closureEvidenceRefs: ["scripts/reverse-agent/live-conflict-arbitration-matrix-gate.mjs", "fixtures/reverse-agent/live-conflict-arbitration-matrix.fixture.json", "provider_backed_eight_window_conflict_matrix", "synthesizer_extended_ten_topic_parse_matrix"],
...
function buildHardeningGapLedger(pillars) {
	...
	const gaps = HARDENING_GAP_CATALOG.map((gap, index) => {
		...
		const artifactBackedClosure = gap.status === "closed" && gap.artifactBackedClosure === true && (gap.closureEvidenceRefs ?? []).length >= 4;
		const closureReady = gap.regressionCommands.length > 0 && Boolean(gap.closureGate) && gap.acceptanceCriteria.length >= 3 && (gap.status === "closed" ? artifactBackedClosure : gap.missingRuntimeProof.length > 0);
		return {
			...
			promotionPolicy: gap.status === "closed" ? "top_autonomous_definition_allowed_after_artifact_backed_closure" : "do_not_mark_top_autonomous_until_closureGate_passes_and_acceptanceCriteria_are_artifact_backed",
	...
	const gapHash = sha256(JSON.stringify(gaps.map(({ seq, gapId, pillar, status, closureGate, regressionCommands, acceptanceCriteria, artifactBackedClosure, closureEvidenceRefs }) => ({ seq, gapId, pillar, status, closureGate, regressionCommands, acceptanceCriteria, artifactBackedClosure, closureEvidenceRefs }))));
	return {
		...
		promotionPolicy: "topAutonomousDefinition remains false while any gap status is not closed; topAutonomousDefinition may be true only when every gap is closed and artifact-backed",
		gapCount: gaps.length,
		closureGateCount,
		highestPriorityOpen: openGaps.length ? Math.min(...openGaps.map((gap) => gap.priority)) : null,
		openGapCount: openGaps.length,
		closedGapCount: gaps.length - openGaps.length,
		gapHash,
		gaps,
	};
}
...
function buildManifest(root) {
	...
	const allHardeningGapsClosed = hardeningGapLedger.gaps.every((gap) => gap.status === "closed" && gap.artifactBackedClosure === true);
	const topAutonomousDefinition = normalUseGuarantee && allHardeningGapsClosed;
	return {
		...
		hardeningGapLedgerSummary: {
			...
			openGapCount: hardeningGapLedger.openGapCount,
			closedGapCount: hardeningGapLedger.closedGapCount,
