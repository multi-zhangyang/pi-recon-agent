#!/usr/bin/env node
/**
 * Release script for repi-monorepo
 *
 * Usage:
...
function bumpOrSetVersion(target) {
	...
	run(`npm version ${target} --no-git-tag-version && npm version ${target} -ws --no-git-tag-version && node scripts/sync-versions.js && npm install --package-lock-only --ignore-scripts`);
	return getVersion();
}
...
console.log("Refreshing release artifacts...");
