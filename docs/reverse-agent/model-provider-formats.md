...
repi model default --provider <id> --model vendor/model-id
...
`model add` 默认在 `models.json` 里写环境变量引用；真实密钥用 `model login` 写入本机 `auth.json`，或者由 shell 环境变量提供。
...
1. 不确定格式时先跑 `repi provider-doctor --base-url <url> --model <id> --api auto`；如果 `openai-responses` 是 `endpoint_not_found` 而 `openai-completions` 通过，就按 Chat Completions 配置。
2. 更新 `~/.repi/agent/models.json`，只写 env 引用，不写 token 明文。
3. `repi --offline --list-models <provider-or-model>`。
4. `--no-tools` 文本 smoke test。
5. `--tools bash` 或窄工具 allowlist 的工具调用 smoke test。
6. 可选远程长跑：`REPI_REMOTE_PROVIDER_API=openai-completions|openai-responses|anthropic-messages npm run gate:remote-provider-longrun -- --live --no-write`。
7. 小型授权本地靶场验证：路由枚举、工具调用、证据抽取、漏洞证明、最终报告。
8. profile guard：
