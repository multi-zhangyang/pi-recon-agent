...
| `scripts/reverse-agent/worker-provider-repair-rollback-unification-gate.mjs` | WorkerProviderRepairRollbackUnificationGateV1 hard-eval：把 provider-worker、re_swarm worker、compound-frontier 和 operator repair 统一到同一 FailureLedgerEventV1 / RepairQueueItemV1 signature、RepairRollbackPolicyV1、retry window closure 和 regression gate refs；对应 `npm run gate:worker-provider-repair-rollback-unification` |
