...
| `repi-profile/memory/*` | 长期经验、索引、自我进化记录；运行时使用 `~/.repi/agent/recon/memory/events.jsonl` / `case-memory.jsonl` / `retrieval-report.json` / `scope-isolation-report.json` / `artifact-scope-filter-report.json` / `orchestrator-report.json` / `store-report.json` 作为 Memory v5 结构化事实源，所有 append 通过 `.store.lock` 与 `transactions/*.json` manifest 提交，`re_replayer` / `re_autofix` / `re_proof_loop` / `re_complete` / 高价值 `re_lane run` / `re_swarm run` 会自动写回 replay/repair/proof/completion/runtime/worker 事件，`re_swarm` 写回 marker 为 `memory-swarm-writeback`；Markdown journal/playbook 是人类可读镜像；`repi-profile/memory/playbooks/index.md` 记录 playbook 质量/年龄/状态，`repi-profile/memory/playbooks/archive/` 存放被淘汰的低质或过旧链路 |
...
- Slash commands:
  ...
  - `/re-context pack|show|resume [target]`：把 mission/evidence/memory/repair 队列（含 `commander_merge_queue`、`commander_merge_budget`、`worker_scoreboard`）固化成 `context_pack` / `context_artifact`；pack 会写入 `artifact_scope_filter` 和 `memory_orchestrator`，阻断 latest artifact side-channel 污染，并把 MemoryOrchestratorV6 的 pre-task retrieve、scope-safe injection、post-tool writeback、compact resume hook、final supervise 状态固化进 ContextPackV2。
...
- LLM tools:
  ...
  - `re_knowledge_graph`：模型可跨 artifacts 构建/查询长期知识图谱，输出 case signatures、相似案例索引、worker 路由与命令策略；先消费 `KnowledgeScopeIsolationV1`，阻断 scope-blocked artifact 进入 command hints/similarity。`re_context pack` 同时嵌入 `ArtifactScopeFilterV1`，让 latest artifact/context artifact_index 旁路也消费同一 scope verdict。
...
`re_memory verify` 写 `store-report.json`，`re_memory repair-index` 从 verified event chain 重建 `case-memory.jsonl`，`re_memory snapshot` 写 `store-snapshot.json`，`re_memory eval` 写 `usefulness-eval.json` 并度量 hit@1、hit@k、MRR、forbiddenLeakRate 和 forbiddenHitIds；`re_memory feedback` 写 `feedback-closure-report.json`，把 injected memory 的成功反馈、失败反馈和 pending writeback 变成 promotionReady/demotionRequired/pending 队列；`re_memory scope [target]` 写 `scope-isolation-report.json`，把每条 MemoryEventV1 的 `MemoryScopeV1` 与当前 mission/session/workspace/branch/route/target 比对，同 scope allow、跨 workspace/target/route block/quarantine、legacy 无 scope manual-review，并在 `re_memory sediment` 前阻断 cross-scope injection；`re_memory supervise` 写 `supervisor-report.json` 与 `lifecycle-board.md`，把沉淀结果治理成 promotion/demotion/quarantine/expire/merge/retain 队列。`re_lane run` 的高价值 runtime 结果会自动写 `memory_auto_writeback`，把 evidence artifact sha256、evidence_quality、self-heal 和 verifier candidate 一起沉淀；`re_swarm run` 的已执行 worker 会自动写 `memory-swarm-writeback`，把 SubagentRuntimeManifestV1、stdout/stderr hash、toolCallDigest、claim/merge artifact、命令和 blocked/success outcome 写回 MemoryStoreV5。`npm run gate:memory-store` 使用 `fixtures/reverse-agent/memory-store.fixture.json` 验证坏 prevHash 阻断、case index 修复、transaction manifest 和 lane runtime auto writeback marker；`npm run gate:memory-swarm-writeback` 使用 `fixtures/reverse-agent/memory-swarm-writeback.fixture.json` 验证 worker 写回数量、artifact 捕获和 plan/merge 非 run 模式不重复写入。`npm run gate:memory-usefulness` 使用 `fixtures/reverse-agent/memory-usefulness.fixture.json` 验证 authz/pwn 正召回、失败/跨 route forbidden memory 不进 topK，以及 child-process 并发 append probe 保持 hash-chain。`npm run gate:memory-feedback-closure` 使用 `schemas/reverse-agent/memory-feedback-closure.schema.json` 与 `fixtures/reverse-agent/memory-feedback-closure.fixture.json` 验证 injected memory 的成功反馈提升、失败反馈降权、pending writeback 和 supervisor demotion。`npm run gate:memory-vector` 使用 `schemas/reverse-agent/memory-vector.schema.json` 与 `fixtures/reverse-agent/memory-vector.fixture.json` 验证本地 hash embedding、OpenAI-compatible embedding provider contract、env-ref API key、explicit remote allow、vector-index/vector-search-report、`memory_vector_rerank`、route-scoped rerank 和 forbidden cross-route vector leak。`npm run gate:memory-supervisor` 使用 `schemas/reverse-agent/memory-supervisor.schema.json` 与 `fixtures/reverse-agent/memory-supervisor.fixture.json` 验证 `MemorySupervisorV1` lifecycle 治理、promotion/demotion/quarantine/merge 和 runtime `re_memory supervise`。
