...
| `scripts/reverse-agent/swarm-provider-manifest-parity-gate.mjs` | SwarmProviderManifestParityGateV1 hard-eval：验证 re_swarm `SubagentRuntimeManifestV1`、WorkerChildSessionRuntimeBatchV1 和 ParallelProviderWorkerMatrixV1 在 workerId、claimRefs、hash、provider env-ref、failure/repair refs 上保持同源，并强制 multi-provider worker 共享 claim/failure/repair merge ledger、provider worker retry/repair rows 绑定 runtime manifest；对应 `npm run gate:swarm-provider-manifest-parity` |
| `scripts/reverse-agent/worker-provider-repair-rollback-unification-gate.mjs` | WorkerProviderRepairRollbackUnificationGateV1 hard-eval：把 provider-worker、re_swarm worker、compound-frontier 和 operator repair 统一到同一 FailureLedgerEventV1 / RepairQueueItemV1 signature、RepairRollbackPolicyV1、retry window closure、regression gate refs、provider-worker live repair matrix 与 multi-attempt completion chain；对应 `npm run gate:worker-provider-repair-rollback-unification` |
| `scripts/reverse-agent/live-conflict-arbitration-matrix-gate.mjs` | LiveConflictArbitrationMatrixGateV1 hard-eval：把 agent-dogfood、re_swarm、compound-frontier 和 provider-worker claim 放入同一 conflict matrix，验证 source coverage、winner evidence JSON query、loser downgrade、orchestration/platform split 与 claim-ledger hash-chain quality；对应 `npm run gate:live-conflict-arbitration-matrix` |
