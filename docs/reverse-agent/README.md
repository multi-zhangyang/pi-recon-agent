...
| `.repi-harness/evidence/relane-specialist-command-pack/*.json` | `re_lane_specialist_pack` / `/re-lane-specialist-pack` 与 `gate:relane-specialist-command-pack` 生成的 `ReLaneSpecialistCommandPackGateV1`：`runtime:re_lane-specialist-command-pack`、route/lane/command pack/analyzer/self-heal/proof-exit bridge |
...
| `scripts/reverse-agent/toolchain-domain-capability-gate.mjs` | ToolchainDomainCapabilityV1 hard-eval：用 runtime `command -v` 发现真实工具，验证 web-api/web-scan/rev-native/pwn/mobile/mobile-ios/pcap-dfir/memory-forensics/firmware-iot/crypto/agent-security/malware-analysis 等领域都有 fallback、playbook marker、command scaffold 和负例；对应 `npm run gate:toolchain-domain-capability` |
| `scripts/reverse-agent/relane-specialist-command-pack-gate.mjs` | ReLaneSpecialistCommandPackGateV1 hard-eval：验证每个专业域都有 `route_to_domain_lane_seed_matrix`、`domain_lane_command_pack_markers`、`specialist_evidence_analyzer_anchor_matrix`、`self_heal_command_fallback_matrix` 和 `proof_exit_bridge_matrix`；对应 `npm run gate:relane-specialist-command-pack` |
| `scripts/reverse-agent/pwn-advanced-capability-gate.mjs` | PwnAdvancedCapabilityGateV1 hard-eval：验证 `pwn_advanced_capability_gate` 的 heap/tcache、format-string、SROP/ret2dlresolve、one_gadget constraint、seccomp/sandbox 命令包、证据锚点、follow-up/self-heal、toolchain proof-exit、测试和 top harness 接入；对应 `npm run gate:pwn-advanced-capability` |
...
| `scripts/reverse-agent/cross-session-multi-compact-matrix-gate.mjs` | CrossSessionMultiCompactMatrixGateV1 hard-eval：把 cross-session resume、multi-compact pressure、provider continuation 和 operator/proof-loop closure 放入同一矩阵，验证八轮 explicit contextPath、contextSha256/artifact hashes、multi-provider env-ref provider continuation matrix、remote provider continuation sample matrix、budget closure、terminal row 不重开和 compact_resume_ledger_cycle_terminal_alignment 每周期 queued→running→terminal 对齐；对应 `npm run gate:cross-session-multi-compact-matrix` |
...
| `scripts/reverse-agent/release-ci-pipeline-gate.mjs` | ReleaseCiPipelineGateV1 hard-eval：审计 `.github/workflows/repi-harness.yml` 与 docs 模板，确认 CI 按 product boundary → closure readiness → capability release bundle → runtime adapter execution → top harness → repo check → no-diff 顺序执行，且不依赖 live provider secrets；对应 `npm run gate:release-ci-pipeline` |
...
| `scripts/reverse-agent/install-global-profile.sh` | 兼容旧命令名；已 deprecated，只调用 isolated profile init + `install-repi.sh`，不复制文件型 SYSTEM/extensions/skills/prompts，不写 `~/.pi/agent` |
...
- Slash commands:
  ...
  - `/re-lane-specialist-pack show [domain]`：查看 `ReLaneSpecialistCommandPackGateV1`，把 route → lane seeds → command pack markers → anchor analyzer → self-heal commands → proof-exit bridge 拉成可执行闭环。
...
- LLM tools:
  ...
  - `re_exploit_lab`：模型可调用的 exploit/PoC 稳定化实验室工具，绑定 PoC inventory、环境 pin、多次 replay、flake triage 和 bundle manifest。
    ...
    - 新增 analyzer anchors：`web scanner scope/crawl/content/template/manual replay anchors`、`iOS IPA inventory anchors`、`iOS Mach-O/class/selector anchors`、`iOS Frida/objection hook anchors`、`iOS network/keychain replay anchors`、`memory forensics image/profile anchors`、`memory forensics process/network anchors`、`memory forensics credential/artifact anchors`、`memory forensics timeline/carve anchors`。
...
`re_toolchain_domain` / `/re-toolchain show|refresh [domain]` 是专业工具链能力层，不是文档口号。它把 Web/API、Web vulnerability scanning、JS signing、native reverse、pwn、Android、iOS/IPA、PCAP/DFIR、Memory forensics、firmware、crypto、cloud/identity、agent-security、malware-analysis、exploit reliability 映射到真实工具、fallback、proof-exit 和下一步命令：
...
### ReLaneSpecialistCommandPackGateV1

`ReLaneSpecialistCommandPackGateV1` / `runtime:re_lane-specialist-command-pack` 负责证明专业路线不是口号：每个 domain 必须有 route matcher、lane seeds、command pack markers、specialist evidence analyzer anchors、self-heal commands、proof-exit bridge。

```text
/re-lane-specialist-pack show
/re-lane-specialist-pack show agent-security
/re-lane-specialist-pack show malware-analysis
re_lane_specialist_pack {"action":"show","domain":"pwn"}
npm run gate:relane-specialist-command-pack
```

闭环 marker：`route_to_domain_lane_seed_matrix` → `domain_lane_command_pack_markers` → `specialist_evidence_analyzer_anchor_matrix` → `self_heal_command_fallback_matrix` → `proof_exit_bridge_matrix`。缺任一层时不能把 `re_lane plan` 的泛化建议当成专业 command pack。


### DomainProofExitClosureV1

`DomainProofExitClosureV1` / `domain_proof_exit_closure` 把工具链矩阵里的 proof-exit 接进完成审计：`re_complete audit` 会根据当前 route 生成专业域退出条件检查，缺口输出 `domain_proof_exit_missing:<domain>:<proof-exit>`，并把下一步转成 `re_lane`、`re_verifier`、`re_proof_loop` 或 `re_bootstrap` 命令。

```text
/re-domain-proof-exit show
/re-domain-proof-exit write pwn
re_domain_proof_exit {"action":"write","domain":"web-api"}
...
| `web-scan` | scope baseline、crawl corpus、scanner finding queue、manual replay verifier |
| `rev-native` | symbol/import map、comparison sink、runtime trace、patch/replay proof |
| `crypto` | parameter derivation、solver script、known-answer test、transform replay |
| `mobile` | manifest/package map、Java/native hook、anti-debug evidence、runtime anchors |
| `mobile-ios` | IPA inventory、Mach-O/class map、Frida/objection hook、network/keychain replay |
| `memory-forensics` | image profile、process/network map、credential/artifact proof、timeline/carve evidence |

marker: `restriction_to_operator_queue`。scope/tool/env/gate 缺口不是最终拒绝，必须变成 bounded operator action：`re_map`、`re_live_browser plan`、`re_lane plan/run`、`re_bootstrap plan`、`re_operator dispatch`、`re_proof_loop`。
