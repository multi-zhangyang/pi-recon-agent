...
`re_toolchain_domain` / `/re-toolchain show|refresh [domain]` 是专业工具链能力层，不是文档口号。它把 Web/API、Web vulnerability scanning、JS signing、native reverse、pwn、Android、iOS/IPA、PCAP/DFIR、Memory forensics、firmware、crypto、cloud/identity、exploit reliability 映射到真实工具、fallback、proof-exit 和下一步命令：
