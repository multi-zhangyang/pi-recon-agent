...
本仓库现在有三种 Pi-RECON 形态，默认只启用独立 `repi`：
...
| `repi` | Pi-RECON 独立产品入口，默认使用 `~/.repi/agent`；源码 wrapper 和 npm/bin 直启都会由 CLI bootstrap 自动启用 `--recon` 隔离参数 |
