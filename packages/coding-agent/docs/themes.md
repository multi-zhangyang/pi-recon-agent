> repi can create themes. Ask it to build one for your setup.
...
REPI loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.repi/agent/themes/*.json`
- Project: `.repi/themes/*.json` (only after the project is trusted)
- Packages: `themes/` directories or `repi.themes` entries in `package.json`
...
On first run, repi detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.repi/agent/themes
vim ~/.repi/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/multi-zhangyang/pi-recon-agent/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, repi reloads it automatically for immediate visual feedback.
...
REPI uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, repi falls back to the nearest approximation.
