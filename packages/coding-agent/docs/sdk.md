> repi can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to repi's agent capabilities. Use it to embed repi in other applications, build custom interfaces, or integrate with automated workflows.
...
```typescript
import { createAgentSession, SessionManager } from "@pi-recon/repi-coding-agent";
...
**Behavior:**
- **Extension commands** (e.g., `/mycommand`): Execute immediately, even during streaming. They manage their own LLM interaction via `repi.sendMessage()`.
...
The `Agent` class (from `@pi-recon/repi-agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.repi/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.repi/extensions/`)
- Project skills:
  - `.repi/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.repi/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.repi/agent/skills/`)
  - `~/.agents/skills/`
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@pi-recon/repi-coding-agent";
...
The `edit` tool returns `details.diff` for REPI's TUI display and `details.patch` as a standard unified patch for SDK consumers.
...
import { createAgentSession, defineTool } from "@pi-recon/repi-coding-agent";
...
Use `defineTool()` for standalone definitions and arrays like `customTools: [myTool]`. Inline `repi.registerTool({ ... })` already infers parameter types correctly.

Custom tools passed via `customTools` are combined with extension-registered tools. Extensions loaded by the ResourceLoader can also register tools via `repi.registerTool()`.
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.repi/agent/extensions/`, `.repi/extensions/`, and settings.json extension sources.
...
**Event Bus:** Extensions can communicate via `repi.events`. Pass a shared `eventBus` to `DefaultResourceLoader` if you need to emit or listen from outside:

```typescript
import { createEventBus, DefaultResourceLoader } from "@pi-recon/repi-coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@pi-recon/repi-coding-agent";
...
1. Global: `~/.repi/agent/settings.json`
2. Project: `<cwd>/.repi/settings.json`
