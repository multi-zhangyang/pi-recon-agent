...
Extensions can register custom model providers via `repi.registerProvider()`. This enables:
...
export default function (repi: ExtensionAPI) {
  ...
  repi.registerProvider("my-provider", {
    name: "My Provider",
...
The extension factory can also be `async`. For dynamic model discovery, fetch and register models in the factory instead of `session_start`. repi waits for the factory before startup continues, so the provider is available during interactive startup and to `repi --list-models`.
...
Use `repi.unregisterProvider(name)` to remove a provider that was previously registered via `repi.registerProvider(name, ...)`:
...
models: [{
  ...
  // ...
  reasoning: true,
  thinkingLevelMap: {              // map repi levels to provider values; null hides unsupported levels
    minimal: null,
    low: null,
    medium: null,
    high: "default",
...
OpenAI-compatible prompt-cache fields and Anthropic-style cache markers are opt-in only; REPI does not infer them from provider/model names. Set `supportsLongCacheRetention: true` only when the endpoint explicitly accepts `prompt_cache_key`/`prompt_cache_retention` or `cache_control.ttl`.
...
```typescript
repi.registerProvider("custom-api", {
  baseUrl: "https://api.example.com",
  apiKey: "$MY_API_KEY",
...
```typescript
import type { OAuthCredentials, OAuthLoginCallbacks } from "@pi-recon/repi-ai";
...
Credentials are persisted in `~/.repi/agent/auth.json`:
...
- [anthropic.ts](https://github.com/multi-zhangyang/pi-recon-agent/blob/main/packages/ai/src/providers/anthropic.ts) - Anthropic Messages API
- [mistral.ts](https://github.com/multi-zhangyang/pi-recon-agent/blob/main/packages/ai/src/providers/mistral.ts) - Mistral Conversations API
- [openai-completions.ts](https://github.com/multi-zhangyang/pi-recon-agent/blob/main/packages/ai/src/providers/openai-completions.ts) - OpenAI Chat Completions
- [openai-responses.ts](https://github.com/multi-zhangyang/pi-recon-agent/blob/main/packages/ai/src/providers/openai-responses.ts) - OpenAI Responses API
- [google.ts](https://github.com/multi-zhangyang/pi-recon-agent/blob/main/packages/ai/src/providers/google.ts) - Google Generative AI
- [amazon-bedrock.ts](https://github.com/multi-zhangyang/pi-recon-agent/blob/main/packages/ai/src/providers/amazon-bedrock.ts) - AWS Bedrock
...
When a request exceeds the model's context window, repi can recover automatically by compacting the conversation and retrying. This recovery only kicks in if repi recognizes the failure as an overflow.

Detection runs on the finalized assistant message:

- `stopReason === "error"`
- `errorMessage` matches one of repi's known overflow patterns (see [`packages/ai/src/utils/overflow.ts`](https://github.com/multi-zhangyang/pi-recon-agent/blob/main/packages/ai/src/utils/overflow.ts))

If your provider returns overflow errors with a message repi does not recognize, normalize the error from the same extension that registers the provider. Use a `message_end` handler to rewrite the assistant message so its `errorMessage` starts with a phrase repi recognizes. The generic fallback `context_length_exceeded` is the safest choice.
...
`message_end` runs before repi tracks the assistant message for auto-compaction, so the rewritten `errorMessage` is what repi checks. With this in place, repi will:
...
- Match a provider-specific pattern, not repi's generic overflow patterns. Rewriting rate-limit or throttling errors (`rate limit`, `too many requests`) would falsely trigger compaction instead of repi's normal retry-with-backoff path.
...
interface ProviderModelConfig {
  ...
  /** Maps repi thinking levels to provider/model-specific values; null marks a level unsupported. */
...
OpenAI-compatible prompt-cache fields and Anthropic-style cache markers are opt-in only; REPI does not infer them from provider/model names.
