...
REPI supports subscription-based providers via OAuth and API key providers via environment variables or auth file. For each provider, repi knows all available models. The list is updated with every repi release.
...
Use `/logout` to clear credentials. Tokens are stored in `~/.repi/agent/auth.json` and auto-refresh when expired.
...
Reference for environment variables and `auth.json` keys: [`const envMap`](https://github.com/multi-zhangyang/pi-recon-agent/blob/main/packages/ai/src/env-api-keys.ts) in [`packages/ai/src/env-api-keys.ts`](https://github.com/multi-zhangyang/pi-recon-agent/blob/main/packages/ai/src/env-api-keys.ts).

#### Auth File

Store credentials in `~/.repi/agent/auth.json`:
...
```bash
repi --provider amazon-bedrock --model us.anthropic.claude-sonnet-4-20250514-v1:0
```
...
repi --provider amazon-bedrock --model arn:aws:bedrock:us-east-1:123456789012:application-inference-profile/abc123
```
...
repi --provider cloudflare-ai-gateway --model "claude-sonnet-4-5"
```
...
For normal repi usage, prefer unified billing or stored BYOK. Inline BYOK requires configuring an additional upstream `Authorization` header for the Cloudflare AI Gateway provider, for example via a `models.json` provider/model override.
...
export CLOUDFLARE_API_KEY=...           # or use /login
export CLOUDFLARE_ACCOUNT_ID=...
repi --provider cloudflare-workers-ai --model "@cf/moonshotai/kimi-k2.6"
```

REPI automatically sets `x-session-affinity` for [prefix caching](https://developers.cloudflare.com/workers-ai/features/prompt-caching/) discounts.
