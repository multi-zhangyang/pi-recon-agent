> repi can create prompt templates. Ask it to build one for your workflow.
...
REPI loads prompt templates from:

- Global: `~/.repi/agent/prompts/*.md`
- Project: `.repi/prompts/*.md` (only after the project is trusted)
- Packages: `prompts/` directories or `repi.prompts` entries in `package.json`
...
- `${N:-default}` for positional arg `N`, using `default` when missing or empty
- `${@:-default}` or `${ARGUMENTS:-default}` for all args, using `default` when empty
