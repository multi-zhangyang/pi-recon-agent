...
| Memory v3 events | `~/.repi/agent/recon/memory/events.jsonl` | Append-only `MemoryEventV1` hash chain written by `re_reflect write`, `re_memory append/evolve`, `re_replayer`, `re_autofix`, `re_proof_loop`, and `re_complete` writebacks |
