...
import {
	highlightCode as highlightCodeWithTheme,
	getLanguageFromPath as resolveLanguageFromPath,
} from "./syntax-highlight.ts";
import type { ThemeBg, ThemeColor, ThemeColorMode, Theme as ThemeContract } from "./theme.ts";

export type { ThemeBg, ThemeColor, ThemeColorMode } from "./theme.ts";
...
const themeStates = new WeakMap<object, ThemeState>();

function getThemeState(instance: object): ThemeState {
	const state = themeStates.get(instance);
...
export class Theme implements ThemeContract {
	...
	constructor(
		...
		const fgColorsMap = new Map<ThemeColor, string>();
		for (const [key, value] of Object.entries(fgColors) as [ThemeColor, string | number][]) {
			fgColorsMap.set(key, fgAnsi(value, mode));
		}
		const bgColorsMap = new Map<ThemeBg, string>();
		for (const [key, value] of Object.entries(bgColors) as [ThemeBg, string | number][]) {
			bgColorsMap.set(key, bgAnsi(value, mode));
		}
		themeStates.set(this, { fgColors: fgColorsMap, bgColors: bgColorsMap, mode });
	}

	fg(color: ThemeColor, text: string): string {
		const ansi = getThemeState(this).fgColors.get(color);
		if (!ansi) throw new Error(`Unknown theme color: ${color}`);
		return `${ansi}${text}\x1b[39m`; // Reset only foreground color
	}

	bg(color: ThemeBg, text: string): string {
		const ansi = getThemeState(this).bgColors.get(color);
	...
	getColorMode(): ColorMode {
		return getThemeState(this).mode;
	}
...
const THEME_KEY = Symbol.for("@pi-recon/repi-coding-agent:theme");
const THEME_KEY_OLD = Symbol.for("@mariozechner/repi-coding-agent:theme");
...
export function highlightCode(code: string, lang?: string): string[] {
	return highlightCodeWithTheme(code, theme, lang);
}
...
export function getMarkdownTheme(): MarkdownTheme {
	return {
		...
		highlightCode: (code: string, lang?: string): string[] => highlightCodeWithTheme(code, theme, lang),
	};
}
