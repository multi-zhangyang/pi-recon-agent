...
Release tarball install uses the four same-version GitHub Release packages together in one command. The internal packages are not published to the npm registry, so installing only the coding-agent tarball fails. `repi-release-manifest.json` lists the complete command, sizes, and SHA-256 hashes.
...
REPI's default provider/model catalog is empty: it neither ships nor loads upstream pi's large catalog. Models exist only when they come from one of these explicit runtime sources:
...
The env-only path can also describe model metadata without creating a file:

```bash
export REPI_MODEL_INPUT="text,image"
export REPI_MODEL_REASONING=true
export REPI_MODEL_THINKING_LEVEL_MAP='{"off":"none","low":"low","medium":"medium","high":"high"}'
...
Headers, compat, and thinking maps use JSON. Header values may reference `$ENV_VAR`; keep secrets out of shell history and committed files. Prices are USD per million tokens. When `input + cacheRead + cacheWrite` exceeds a tier's `inputTokensAbove`, the highest exceeded tier applies to the whole request.

**Custom providers & models:** Add persistent providers via `~/.repi/agent/models.json`; provider and model entries can define API/base URL, headers, compat, input modalities, reasoning/thinking mapping, context/output limits, and four-way/tiered pricing. For custom APIs or OAuth, use extensions. See [REPI runtime configuration](docs/repi-runtime-configuration.md), [docs/models.md](docs/models.md), and [docs/custom-provider.md](docs/custom-provider.md).
...
- **Runtime adapters:** `re_runtime_adapter` infers CDP/tshark/Frida/binwalk/GDB/r2/pwntools runners from target shape and writes parsed evidence artifacts.
...
| Option | Description |
|--------|-------------|
| `--provider <name>` | Explicitly configured provider id |
...
# Different explicitly configured provider/model
repi --provider gateway --model provider/model-id "分析 Web/API 授权状态机"

# Model with provider prefix (no --provider needed)
repi --model gateway/provider-model "生成 exploit-lab 复现矩阵"

# Model with thinking level shorthand
repi --model gateway/provider-model:high "构建 pwn 目标的 leak→primitive→proof 路线"

# Limit model cycling
repi --models "gateway/*,local/model-id:low"
...
| `REPI_MODEL_REASONING` | Whether the model supports reasoning (`1`/`true`/`yes` or `0`/`false`/`no`) |
| `REPI_MODEL_THINKING_LEVEL_MAP` | JSON map from REPI thinking levels to provider-specific values; `null` marks a level unsupported |
| `REPI_HEADERS` / `REPI_MODEL_HEADERS` | JSON provider/model request headers; values may use `$ENV_VAR`, and `null` removes a header |
| `REPI_COMPAT` / `REPI_MODEL_COMPAT` | JSON provider/model wire-compatibility overrides |
| `REPI_AUTH_HEADER` | Add `Authorization: Bearer <token>` from the resolved env-only token when true |
| `REPI_MODEL_COST_INPUT` / `REPI_MODEL_COST_OUTPUT` | Input/output price in USD per million tokens |
| `REPI_MODEL_COST_CACHE_READ` / `REPI_MODEL_COST_CACHE_WRITE` | Cache read/write price in USD per million tokens |
| `REPI_MODEL_COST_TIERS` | JSON request-wide tiers; the highest threshold exceeded by total input usage applies |
