...
Content`,
		...
		it("should auto-discover root markdown skills from .repi skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .repi", async () => {
			const extDir = join(tempDir, ".repi", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".repi"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".repi", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".repi", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".repi", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".repi", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".repi", "prompts");
			mkdirSync(promptsDir, { recursive: true });
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user .repi/agent skills", async () => {
		...
		it("should use the project .repi dir as baseDir for project .repi skills", async () => {
			const projectBaseDir = join(tempDir, ".repi");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".repi", "agent");
		...
		it("should dedupe user skill entries when ~/.repi/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .repi auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".repi\n");

			const skillDir = join(tempDir, ".repi", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("command spawning", () => {
		it("should preserve argv entries containing spaces", () => {
			...
			const valueWithSpace = "C:\\Users\\A B\\.repi\\npm";
	...
	describe("npmCommand", () => {
		it("should use npmCommand argv for npm installs", async () => {
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				...
				// (opt #123) managed npm install now carries a wall timeout so a hung
				// registry / credential prompt can't freeze the install flow.
				{ timeoutMs: 300000 },
			);
		});
		...
		it("should use bun --cwd for npm package installs", async () => {
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				...
				// (opt #123) same hang bound as the npm-cli path.
				{ timeoutMs: 300000 },
			);
		});

		it("should bound npm uninstall with a wall timeout (opt #123)", async () => {
			// uninstallNpm early-returns when the install root does not exist, so
			// materialize it to reach the runNpmCommand call site.
		...
		it("should bound bun uninstall with a wall timeout (opt #123)", async () => {
			settingsManager = SettingsManager.inMemory({
				npmCommand: ["mise", "exec", "bun@1", "--", "bun"],
			});
			packageManager = new DefaultPackageManager({
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"mise",
				["exec", "bun@1", "--", "bun", "uninstall", "@scope/pkg", "--cwd", join(agentDir, "npm")],
				{ timeoutMs: 300000 },
			);
		});
		...
		it("should reconcile an existing git checkout to a pinned ref during install", async () => {
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"git",
				["fetch", "origin", "v2"],
				expect.objectContaining({ cwd: targetDir, env: { GIT_TERMINAL_PROMPT: "0" } }),
			);
			expect(runCommandSpy).toHaveBeenCalledWith(
				"git",
		...
		it("should use plain install for git package dependencies when npmCommand is configured", async () => {
			...
			expect(runCommandSpy).toHaveBeenCalledWith("pnpm", ["install"], { cwd: targetDir, timeoutMs: 300000 });
		});

		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".repi", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .repi settings base", () => {
			...
			const rel = relative(join(tempDir, ".repi"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
	...
	describe("offline mode and network timeouts", () => {
		it("should update project npm packages using @latest when newer version is available", async () => {
			const installedPath = join(tempDir, ".repi", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@latest", "--prefix", join(tempDir, ".repi", "npm"), "--legacy-peer-deps"],
				// (opt #123) single-package update routes through installNpmBatch →
				// now carries the wall timeout.
				{ timeoutMs: 300000 },
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".repi", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".repi", "npm", "node_modules", "project-current");
			...
			expect(runCommandSpy).toHaveBeenNthCalledWith(
				...
				// (opt #123) batched npm install carries the wall timeout.
				{ timeoutMs: 300000 },
			);
			expect(runCommandSpy).toHaveBeenNthCalledWith(
				2,
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".repi", "npm", "node_modules", "example");
