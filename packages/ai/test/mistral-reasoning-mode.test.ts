...
describe("Mistral reasoning mode selection", () => {
	it("uses reasoning_effort for Mistral Small 4", async () => {
		const payload = await capturePayload(getModel<"mistral-conversations">("mistral", "mistral-small-2603")!, {
			reasoning: "medium",
		});
	...
	it("uses prompt_mode for Magistral reasoning models", async () => {
		const payload = await capturePayload(getModel<"mistral-conversations">("mistral", "magistral-medium-latest")!, {
			reasoning: "medium",
		});
	...
	it("uses reasoning_effort for Mistral Medium 3.5", async () => {
		const payload = await capturePayload(getModel<"mistral-conversations">("mistral", "mistral-medium-3.5")!, {
			reasoning: "medium",
		});
