...
describe("Anthropic temperature compatibility", () => {
	it("omits temperature for Claude Opus 4.7", async () => {
		const payload = await capturePayload(getModel<"anthropic-messages">("anthropic", "claude-opus-4-7")!, {
			temperature: 0,
		});
	...
	it("keeps temperature for Claude Sonnet 4.6", async () => {
		const payload = await capturePayload(getModel<"anthropic-messages">("anthropic", "claude-sonnet-4-6")!, {
			temperature: 0,
		});
