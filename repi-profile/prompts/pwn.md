---
description: 启动 REPI Pwn exploit 工程工作流
...
4. 跑 `pwn-primitive-offset-analyzer`：从 RIP/EIP/PC 或 `REPI_CRASH_VALUE` 得到 `pwn cyclic offset anchors`。
5. 跑 `pwn-primitive-rop-libc-scaffold`：提取 PLT/GOT、pop gadgets、libc 指纹，形成 `pwn ROP/libc chain anchors`。
6. 跑 `pwn-primitive-local-verifier`：用 `REPI_OFFSET` 或 `REPI_PAYLOAD_HEX` 做本地 payload smoke，形成 `pwn local verifier anchors`。
