...
3. 需要动态证据时调用 `re_mobile_runtime run <apk-or-package> [packageName] [timeout-ms]`；默认只生成观测与 hook 模板，只有明确需要 live Frida attach 时设置 `REPI_MOBILE_ATTACH=1`。
