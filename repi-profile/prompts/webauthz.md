...
3. 需要动态证据时调用 `re_web_authz_state run <url> [timeout-ms]`；设置 `COOKIE_A/COOKIE_B` 或 `AUTH_A/AUTH_B` 区分主体，设置 `REPI_OBJECT_A/REPI_OBJECT_B` 做 IDOR/BOLA 对照。
4. 变更型 rollback 默认关闭；只有明确需要时设置 `REPI_AUTHZ_MUTATE=1`、`REPI_MUTATION_URL`、`REPI_MUTATION_BODY`、`REPI_RESTORE_BODY`。
