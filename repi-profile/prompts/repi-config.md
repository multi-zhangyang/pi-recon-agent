...
5. 网关格式不确定时先按 `openai-completions` 写入配置，用 `repi model doctor` 做离线解析，再用 `repi model test --provider <id> --model <model-id>` 做最小真实调用；如果上游明确要求 `/v1/responses` 或 Anthropic Messages，再把 `api` 改成对应格式。
