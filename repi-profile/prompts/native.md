...
3. 需要动态证据时调用 `re_native_runtime run <elf-or-so> [timeout-ms]`；默认只生成观测与 GDB/pwntools 模板，只有明确需要 live GDB execution 时设置 `REPI_NATIVE_RUN=1`，可用 `REPI_NATIVE_ARGS` 传入参数。
